...
if __name__ == "__main__":
    ...
    # ---- LOAD DATA ----
    file = "Fluo_Decay/deux_TP_HugoEnzo_spads1_2025-12-02T16_32_23.csv"
    Histo, bin_values = get_Histo_from_csv(file)
    ...
    # ---------------------------------------------------------
    # FIT 1: MONO-EXP (Unweighted)
    # ---------------------------------------------------------
    def func_mono(t, o, a, tau): return model_mono(t, [o, a, tau])
    popt1, pcov1 = curve_fit(func_mono, t, histogram, p0=p0_mono)
    perr1 = np.sqrt(np.diag(pcov1))
    
    # Calculate Chi2 (Sum Squared Error)
    res1 = histogram - model_mono(t, popt1)
    chi2_1 = np.sum(res1**2)

    print_result("1. Mono-Exp (Unweighted)", popt1, perr1, ['Off', 'A', 'tau'], chi2_1, n_bins)
    plot_result(t, histogram, model_mono(t, popt1), "Mono (Unweighted)")

    # ---------------------------------------------------------
    # FIT 2: BI-EXP (Unweighted)
    # ---------------------------------------------------------
    p0_bi = [popt1[0], popt1[1]*0.9, popt1[2], popt1[1]*0.1, 2.0]
    def func_bi(t, o, a1, t1, a2, t2): return model_bi(t, [o, a1, t1, a2, t2])
    
    try:
        popt2, pcov2 = curve_fit(func_bi, t, histogram, p0=p0_bi)
        perr2 = np.sqrt(np.diag(pcov2))
        
        res2 = histogram - model_bi(t, popt2)
        chi2_2 = np.sum(res2**2)

        print_result("2. Bi-Exp (Unweighted)", popt2, perr2, ['Off', 'A1', 't1', 'A2', 't2'], chi2_2, n_bins)
        plot_result(t, histogram, model_bi(t, popt2), "Bi-Exp (Unweighted)")
        
    except Exception as e:
        print(f"Bi-exponential fit failed: {e}")
